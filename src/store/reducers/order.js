...
const reducer = (state = initialState, action) => {
  switch(action.type) {
    ...
    case types.PURCHASE_BURGER_FAIL: return purchaseBurgerFail(state, action);
    case types.FETCH_ORDERS_START: return fetchOrdersStart( state, action );
    case types.FETCH_ORDERS_SUCCESS: return fetchOrdersSuccess( state, action );
    case types.FETCH_ORDERS_FAIL: return fetchOrdersFail( state, action );
